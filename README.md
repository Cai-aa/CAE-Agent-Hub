...
| [CalculiX MCP](MCP/CalculiX) | Active | Wrap the open-source CalculiX FEM solver (`ccx`, GPLv2, no license): parse a `.inp` deck, edit design variables in place, run ccx fire-and-forget, read `.dat` text results, export them to `result_mesh.json` for the viewer, run two-stage sizing optimization (LHS + coordinate descent) to minimize mass, and extract natural frequencies (`*FREQUENCY`) with viewer-renderable mode shapes. | `mcp_server.py`, `tools/inp_parser.py`, `tools/solver.py`, `tools/result_exporter.py`, `tools/optimizer.py` | CalculiX, FEM, ccx, .inp, .dat, .frd, von Mises, design variables, sizing optimization, modal analysis |
